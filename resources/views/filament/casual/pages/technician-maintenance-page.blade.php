...
<div class="flex flex-col bg-blue-600" style="min-height:100dvh">
    ...
    <main class="flex-1 overflow-y-auto rounded-t-3xl bg-gray-50 pb-28 pt-6 dark:bg-gray-950">
        ...
        <div class="rounded-2xl bg-white p-4 ring-1 ring-black/5 dark:bg-gray-900 dark:ring-white/10">
            <p class="mb-3 text-sm font-semibold text-gray-800 dark:text-gray-100">Mulai pemeriksaan</p>
        ...
        <button wire:click="startCurrentMonth" wire:loading.attr="disabled" class="flex w-full items-center justify-center gap-2 rounded-2xl bg-blue-600 py-3.5 text-sm font-semibold text-white shadow-sm transition active:scale-[0.98] disabled:opacity-60">
        ...
        @forelse($this->maintenances as $maintenance)
            <a href="{{ \App\Filament\Casual\Pages\TechnicianMaintenanceDetailPage::getUrl(['record' => $maintenance->id], panel: 'casual') }}" class="flex items-center gap-3 border-t border-gray-100 p-4 transition active:bg-gray-50 dark:border-gray-800 dark:active:bg-gray-800">
                <span class="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-xl bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-300"><svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.7"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 21h16.5M5.25 21V5.25A2.25 2.25 0 0 1 7.5 3h9a2.25 2.25 0 0 1 2.25 2.25V21M8.25 7.5h7.5M8.25 11.25h7.5M8.25 15h4.5"/></svg></span>
                <span class="min-w-0 flex-1"><span class="block truncate font-semibold text-gray-900 dark:text-white">{{ $maintenance->branch->name }}</span><span class="mt-1 block text-xs text-gray-400">{{ $maintenance->maintenance_month }}/{{ $maintenance->maintenance_year }} · Nilai {{ $maintenance->score }}%</span></span>
