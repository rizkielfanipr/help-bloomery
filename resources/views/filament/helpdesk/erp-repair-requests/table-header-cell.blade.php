...
@once
    <span
        class="hidden"
        x-init="
            if (window.erpRepairRequestSelectionSyncInstalled) return
            window.erpRepairRequestSelectionSyncInstalled = true
...
<th class="fi-ta-header-cell align-top" @if($width) style="width: {{ $width }}" @endif>
    ...
    @switch($name)
        @case('ticket_number')
            <input wire:model.live.debounce.500ms="tableFilters.ticket_number.value" type="search" placeholder="Cari tiket..." class="{{ $inputClass }}">
            @break
        ...
        @case('keterangan')
            <input wire:model.live.debounce.500ms="tableFilters.keterangan.value" type="search" placeholder="Cari keterangan..." class="{{ $inputClass }}">
            @break

        @case('status')
            <select wire:model.live="tableFilters.status.value" class="{{ $inputClass }}">
                <option value="">- Semua Status -</option>
                @foreach(ItRequestStatus::cases() as $status)
        ...
        @case('priority')
            <select wire:model.live="tableFilters.priority.value" class="{{ $inputClass }}">
