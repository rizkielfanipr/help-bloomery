        ...
        <div class="mx-5 rounded-2xl border border-gray-200 bg-white p-4 dark:border-white/10 dark:bg-gray-900">
            ...
            @if($this->trips->isNotEmpty())
                <button wire:click="downloadPdf"
                        class="mt-4 flex w-full items-center justify-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5 text-sm font-semibold text-blue-700 transition active:scale-[0.99] dark:border-blue-800/50 dark:bg-blue-900/20 dark:text-blue-400">
        ...
        <div class="mx-5 mt-4 space-y-3">
            @foreach($this->tripDays as $day)
                <div class="flex items-center justify-between px-1 pt-2">
                    ...
                    <span class="rounded-full px-2.5 py-1 text-xs font-medium {{ $day['trips']->isNotEmpty() ? 'bg-blue-50 text-blue-700 dark:bg-blue-900/20 dark:text-blue-400' : 'bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400' }}">
                        {{ $day['trips']->isNotEmpty() ? $day['trips']->count().' perjalanan' : 'Belum ada input' }}
                    </span>
                </div>
                @forelse($day['trips'] as $trip)
                <a href="{{ \App\Filament\Casual\Pages\TripDetail::getUrl(['trip' => $trip->id]) }}"
