        ...
        <div class="rounded-xl border border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50 p-6 dark:border-blue-900/50 dark:from-blue-950/40 dark:to-indigo-950/40">
            <div class="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
                ...
                <div class="flex gap-2">
                    <a href="{{ \App\Filament\Helpdesk\Pages\BillOfMaterialPage::getUrl() }}" class="rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-200">Kembali</a>
                    @if(auth()->user()?->hasRole('SUPERADMIN') || auth()->user()?->can('edit bill of materials'))
                        <a href="{{ \App\Filament\Helpdesk\Pages\EditBomRecipePage::getUrl(['bom' => $bomId]) }}" class="rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700">Update BOM</a>
                    @endif
                </div>
            </div>
        </div>
