...
<div class="mx-auto w-full max-w-6xl space-y-5">
    ...
    <section class="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
        <div class="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
            ...
            <p class="mt-1 text-xs text-gray-400">Revenue ÷ pax untuk setiap staff yang bertugas pada shift tersebut. Angka dihitung otomatis dari jam shift lengkap di master cabang setelah shift berakhir.</p>
        </div>
            ...
            <table class="w-full min-w-[900px] text-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @forelse($record->basketSizeRecords->sortBy('shift_number') as $basket)
                        <tr><td class="px-5 py-3"><p class="font-bold">{{ $basket->shift_name }}</p><p class="text-xs text-gray-400">{{ substr($basket->shift_start_time, 0, 5) }}–{{ substr($basket->shift_end_time, 0, 5) }}</p>@unless($basket->isFinal())<p class="mt-1 inline-flex rounded-full border border-amber-200 bg-amber-50 px-2 py-0.5 text-[10px] font-semibold text-amber-700 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-300" title="Dihitung otomatis dari jam shift lengkap setelah shift berakhir">Sementara</p>@endunless</td><td class="px-5 py-3 text-right">{{ $fmt($basket->revenue) }}</td><td class="px-5 py-3 text-right">{{ number_format($basket->total_pax) }}</td><td class="px-5 py-3 text-right font-bold">{{ $basket->basket_size !== null ? $fmt($basket->basket_size) : 'Pax 0' }}</td><td class="px-5 py-3">@foreach($basket->employeeRecords as $employee)<p>{{ $employee->employee_name }} <span class="font-bold text-blue-600">· {{ $employee->basket_size_credit !== null ? $fmt($employee->basket_size_credit) : '—' }}</span></p>@endforeach</td></tr>
                    @empty
