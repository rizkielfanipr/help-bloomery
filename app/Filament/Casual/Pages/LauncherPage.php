<?php

namespace App\Filament\Casual\Pages;

use App\Filament\Casual\Resources\ServiceRequests\ServiceRequestResource;
use App\Models\ContentRequest;
use App\Models\DesignRequest;
use App\Models\ErpRepairRequest;
use App\Models\PurchaseRequest;
use Filament\Pages\Page;
use Illuminate\Support\Collection;

class LauncherPage extends Page
{
    protected string $view = 'filament.casual.pages.launcher-page';

    protected static string $layout = 'filament.casual.layouts.bare';

    protected static bool $shouldRegisterNavigation = false;

    public function getTitle(): string
    {
        return 'Beranda';
    }

    public function getBreadcrumbs(): array
    {
        return [];
    }

    public function tiles(): array
    {
        $user = auth()->user();

        $all = [
            [
                'label' => 'Absensi',
                'iconBg' => 'bg-blue-50',
                'iconColor' => 'text-blue-500',
                'path' => 'M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z',
                'href' => fn (): string => route('filament.casual.pages.clock-page'),
                'permission' => 'access employee app attendance',
            ],
            [
                'label' => 'Driver',
                'iconBg' => 'bg-emerald-50',
                'iconColor' => 'text-emerald-500',
                'path' => 'M8.25 18.75a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 0 1-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 0 0-3.213-9.193 2.056 2.056 0 0 0-1.58-.86H14.25M16.5 18.75h-2.25m0-11.177v-.958c0-.568-.422-1.048-.987-1.106a48.554 48.554 0 0 0-10.026 0 1.106 1.106 0 0 0-.987 1.106v7.635m12-6.677v6.677m0 4.5v-4.5m0 0h-12',
                'href' => fn (): string => TripDashboard::getUrl(panel: 'casual'),
                'permission' => 'access employee app driver',
            ],
            [
                'label' => 'Teknisi',
                'iconBg' => 'bg-orange-50',
                'iconColor' => 'text-orange-500',
                'path' => 'M11.42 15.17 17.25 21A2.652 2.652 0 0 0 21 17.25l-5.877-5.877M11.42 15.17l2.496-3.03c.317-.384.74-.626 1.208-.766M11.42 15.17l-4.655 5.653a2.548 2.548 0 1 1-3.586-3.586l6.837-5.63m5.108-.233c.55-.164 1.163-.188 1.743-.14a4.5 4.5 0 0 0 4.486-6.336l-3.276 3.277a3.004 3.004 0 0 1-2.25-2.25l3.276-3.276a4.5 4.5 0 0 0-6.336 4.486c.091 1.076-.071 2.264-.904 2.95l-.102.085m-1.745 1.437L5.909 7.5H4.5L2.25 3.75l1.5-1.5L7.5 4.5v1.409l4.26 4.26m-1.745 1.437 1.745-1.437m6.615 8.206L15.75 15.75M4.867 19.125h.008v.008h-.008v-.008Z',
                'href' => fn (): string => ServiceRequestResource::getUrl('index', panel: 'casual'),
                'permission' => 'access employee app technician',
            ],
            [
                'label' => 'Req. Teknisi',
                'iconBg' => 'bg-sky-50',
                'iconColor' => 'text-sky-500',
                'path' => 'M11.42 15.17 17.25 21A2.652 2.652 0 0 0 21 17.25l-5.877-5.877M11.42 15.17l2.496-3.03c.317-.384.74-.626 1.208-.766M11.42 15.17l-4.655 5.653a2.548 2.548 0 1 1-3.586-3.586l6.837-5.63m5.108-.233c.55-.164 1.163-.188 1.743-.14a4.5 4.5 0 0 0 4.486-6.336l-3.276 3.277a3.004 3.004 0 0 1-2.25-2.25l3.276-3.276a4.5 4.5 0 0 0-6.336 4.486c.091 1.076-.071 2.264-.904 2.95l-.102.085m-1.745 1.437L5.909 7.5H4.5L2.25 3.75l1.5-1.5L7.5 4.5v1.409l4.26 4.26m-1.745 1.437 1.745-1.437m6.615 8.206L15.75 15.75M4.867 19.125h.008v.008h-.008v-.008Z',
                'href' => fn (): string => TechnicianRequestPage::getUrl(panel: 'casual'),
                'permission' => 'access employee app technician request',
            ],
            [
                'label' => 'Briefing',
                'iconBg' => 'bg-violet-50',
                'iconColor' => 'text-violet-500',
                'path' => 'M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 0 0-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 0 0 .75-.75 2.25 2.25 0 0 0-.1-.664m-5.8 0A2.251 2.251 0 0 1 13.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25ZM6.75 12h.008v.008H6.75V12Zm0 3h.008v.008H6.75V15Zm0 3h.008v.008H6.75V18Z',
                'href' => fn (): string => route('filament.casual.pages.daily-briefing-page'),
                'permission' => 'access employee app briefing',
            ],
            [
                'label' => 'Sales Report',
                'iconBg' => 'bg-teal-50',
                'iconColor' => 'text-teal-500',
                'path' => 'M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75ZM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625ZM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z',
                'href' => fn (): string => route('filament.casual.pages.sales-report-page'),
                'permission' => 'access employee app sales report',
            ],
            [
                'label' => 'Stock Card',
                'iconBg' => 'bg-blue-50',
                'iconColor' => 'text-blue-500',
                'path' => 'M20.25 6.375c0 2.278-3.694 4.125-8.25 4.125S3.75 8.653 3.75 6.375m16.5 0c0-2.278-3.694-4.125-8.25-4.125S3.75 4.097 3.75 6.375m16.5 0v11.25c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125V6.375m16.5 2.625c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125m16.5 5.625c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125',
                'href' => fn (): string => route('filament.casual.pages.stock-card-page'),
                'permission' => 'access employee app stock card',
            ],
            [
                'label' => 'Purchasing',
                'iconBg' => 'bg-amber-50',
                'iconColor' => 'text-amber-500',
                'path' => 'M15.75 10.5V6a3.75 3.75 0 1 0-7.5 0v4.5m11.356-1.993 1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 0 1-1.12-1.243l1.264-12A1.125 1.125 0 0 1 5.513 7.5h12.974c.576 0 1.059.435 1.119 1.007ZM8.625 10.5a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm7.5 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Z',
                'href' => fn (): string => route('filament.casual.pages.purchase-request-page'),
                'permission' => 'access employee app purchasing',
            ],
            [
                'label' => 'Desain',
                'iconBg' => 'bg-pink-50',
                'iconColor' => 'text-pink-500',
                'path' => 'M9.813 15.904 9 18.75l-.813-2.846a4.5 4.5 0 0 0-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 0 0 3.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 0 0 3.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 0 0-3.09 3.09ZM18.259 8.715 18 9.75l-.259-1.035a3.375 3.375 0 0 0-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 0 0 2.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 0 0 2.456 2.456L21.75 6l-1.035.259a3.375 3.375 0 0 0-2.456 2.456ZM16.894 20.567 16.5 21.75l-.394-1.183a2.25 2.25 0 0 0-1.423-1.423L13.5 18.75l1.183-.394a2.25 2.25 0 0 0 1.423-1.423l.394-1.183.394 1.183a2.25 2.25 0 0 0 1.423 1.423l1.183.394-1.183.394a2.25 2.25 0 0 0-1.423 1.423Z',
                'href' => fn (): string => route('filament.casual.pages.design-request-page'),
                'permission' => 'access employee app design',
            ],
            [
                'label' => 'Konten',
                'iconBg' => 'bg-fuchsia-50',
                'iconColor' => 'text-fuchsia-500',
                'path' => 'm15.75 10.5 4.72-4.72a.75.75 0 0 1 1.28.53v11.38a.75.75 0 0 1-1.28.53l-4.72-4.72M4.5 18.75h9a2.25 2.25 0 0 0 2.25-2.25v-9a2.25 2.25 0 0 0-2.25-2.25h-9A2.25 2.25 0 0 0 2.25 7.5v9a2.25 2.25 0 0 0 2.25 2.25Z',
                'href' => fn (): string => route('filament.casual.pages.content-request-page'),
                'permission' => 'access employee app content',
            ],
            [
                'label' => 'ERP',
                'iconBg' => 'bg-indigo-50',
                'iconColor' => 'text-indigo-500',
                'path' => 'M20.25 6.375c0 2.278-3.694 4.125-8.25 4.125S3.75 8.653 3.75 6.375m16.5 0c0-2.278-3.694-4.125-8.25-4.125S3.75 4.097 3.75 6.375m16.5 0v11.25c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125V6.375m16.5 2.625c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125m16.5 5.625c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125',
                'href' => fn (): string => ErpRequestPage::getUrl(panel: 'casual'),
                'permission' => 'access employee app erp',
            ],
        ];

        return array_values(array_map(function (array $tile): array {
            $tile['href'] = $tile['href']();

            return $tile;
        }, array_filter($all, fn (array $tile): bool => $user->can($tile['permission']))));
    }

    /** @return Collection<int, array<string, mixed>> */
    public function recentRequests(): Collection
    {
        $user = auth()->user();
        $results = collect();

        DesignRequest::where('requester_id', $user->id)
            ->latest()->take(3)->get()
            ->each(fn (DesignRequest $r) => $results->push([
                'type' => 'Request Desain',
                'label' => $r->judul_permintaan,
                'iconBg' => 'bg-pink-50',
                'iconColor' => 'text-pink-500',
                'path' => 'M9.813 15.904 9 18.75l-.813-2.846a4.5 4.5 0 0 0-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 0 0 3.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 0 0 3.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 0 0-3.09 3.09Z',
                'status_label' => $r->status->getLabel(),
                'status_color' => $r->status->getColor(),
                'date' => $r->created_at,
            ]));

        ContentRequest::where('requester_id', $user->id)
            ->latest()->take(3)->get()
            ->each(fn (ContentRequest $r) => $results->push([
                'type' => 'Request Konten',
                'label' => $r->judul_konten,
                'iconBg' => 'bg-fuchsia-50',
                'iconColor' => 'text-fuchsia-500',
                'path' => 'm15.75 10.5 4.72-4.72a.75.75 0 0 1 1.28.53v11.38a.75.75 0 0 1-1.28.53l-4.72-4.72M4.5 18.75h9a2.25 2.25 0 0 0 2.25-2.25v-9a2.25 2.25 0 0 0-2.25-2.25h-9A2.25 2.25 0 0 0 2.25 7.5v9a2.25 2.25 0 0 0 2.25 2.25Z',
                'status_label' => $r->status->getLabel(),
                'status_color' => $r->status->getColor(),
                'date' => $r->created_at,
            ]));

        ErpRepairRequest::where('requester_id', $user->id)
            ->with('module')->latest()->take(3)->get()
            ->each(fn (ErpRepairRequest $r) => $results->push([
                'type' => 'Request ERP',
                'label' => $r->module?->name ?? 'Permintaan ERP',
                'iconBg' => 'bg-indigo-50',
                'iconColor' => 'text-indigo-500',
                'path' => 'M20.25 6.375c0 2.278-3.694 4.125-8.25 4.125S3.75 8.653 3.75 6.375m16.5 0c0-2.278-3.694-4.125-8.25-4.125S3.75 4.097 3.75 6.375m16.5 0v11.25c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125V6.375m16.5 2.625c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125m16.5 5.625c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125',
                'status_label' => $r->status->getLabel(),
                'status_color' => $r->status->getColor(),
                'date' => $r->created_at,
            ]));

        PurchaseRequest::where('user_id', $user->id)
            ->latest()->take(3)->get()
            ->each(fn (PurchaseRequest $r) => $results->push([
                'type' => 'Request Purchasing',
                'label' => $r->item_name,
                'iconBg' => 'bg-amber-50',
                'iconColor' => 'text-amber-500',
                'path' => 'M15.75 10.5V6a3.75 3.75 0 1 0-7.5 0v4.5m11.356-1.993 1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 0 1-1.12-1.243l1.264-12A1.125 1.125 0 0 1 5.513 7.5h12.974c.576 0 1.059.435 1.119 1.007ZM8.625 10.5a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm7.5 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Z',
                'status_label' => $r->status->getLabel(),
                'status_color' => $r->status->getColor(),
                'date' => $r->created_at,
            ]));

        return $results->sortByDesc('date')->take(5)->values();
    }
}
