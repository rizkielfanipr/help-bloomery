...
@once
    <span
        class="hidden"
        x-init="
            if (window.designRequestSelectionSyncInstalled) return
            window.designRequestSelectionSyncInstalled = true
...
<th class="fi-ta-header-cell align-top" @if($width) style="width: {{ $width }}" @endif>
    ...
    @switch($name)
        ...
        @case('judul_permintaan')
            <input wire:model.live.debounce.500ms="tableFilters.judul_permintaan.value" type="search" placeholder="Cari judul..." class="{{ $inputClass }}">
            @break

        @case('category.name')
            <select wire:model.live="tableFilters.design_category_id.value" class="{{ $inputClass }}">
                <option value="">- Semua Kategori -</option>
                @foreach(DesignCategory::where('is_active', true)->orderBy('sort_order')->pluck('name', 'id') as $catId => $catName)
        ...
        @case('status')
            <select wire:model.live="tableFilters.status.value" class="{{ $inputClass }}">
                ...
                @foreach(DesignRequestStatus::cases() as $status)
