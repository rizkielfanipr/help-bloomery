<x-filament-panels::page>
    ...
    <div class="space-y-6">
        <section class="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
            ...
            <div class="flex items-start gap-2 border-t border-gray-200 bg-gray-50/60 px-5 py-3 text-xs leading-5 text-gray-500 dark:border-gray-700 dark:bg-gray-800/30 dark:text-gray-400 sm:px-6"><x-heroicon-o-information-circle class="mt-0.5 h-4 w-4 shrink-0 text-blue-500" /><span>Hanya Completed bernilai 100 per hari. Draft, Supervisor Review, Finance Review, Rejected, Rejected by System, dan tidak input bernilai 0. Bulan berjalan dihitung sampai kemarin.</span></div>
        </section>
            ...
            <x-rnd.picker-modal title="Rincian Harian · {{ $detail['name'] }}" description="{{ $periodLabel }} · Satu tanggal bernilai maksimal 100. Shift mengikuti konfigurasi cabang saat ini.">
                ...
                <div class="overflow-y-auto p-5"><div class="space-y-2">
                    @foreach($detail['days'] as $day)
                        @php
                            $statusLabel = ['pending_finance'=>'Finance Review','completed'=>'Completed','rejected'=>'Rejected','rejected_by_system'=>'Rejected by System','draft'=>'Draft','pending_supervisor'=>'Menunggu Supervisor','missing'=>'Tidak Input'][$day['status']] ?? $day['status'];
                            $statusColor = !$day['required'] ? 'bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400' : ($day['score'] === 100 ? 'bg-green-50 text-green-700 dark:bg-green-950/40 dark:text-green-300' : (in_array($day['status'], ['rejected', 'rejected_by_system'], true) ? 'bg-red-50 text-red-700 dark:bg-red-950/40 dark:text-red-300' : 'bg-amber-50 text-amber-700 dark:bg-amber-950/40 dark:text-amber-300'));
                        @endphp
