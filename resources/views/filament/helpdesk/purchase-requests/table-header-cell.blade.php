...
@once
    <span
        class="hidden"
        x-init="
            if (window.purchaseRequestSelectionSyncInstalled) return
            window.purchaseRequestSelectionSyncInstalled = true
...
<th class="fi-ta-header-cell align-top" @if($width) style="width: {{ $width }}" @endif>
    ...
    @switch($name)
        @case('request_code')
            <input wire:model.live.debounce.500ms="tableFilters.request_code.value" type="search" placeholder="Cari kode..." class="{{ $inputClass }}">
            @break
        ...
        @case('item_name')
            <input wire:model.live.debounce.500ms="tableFilters.item_name.value" type="search" placeholder="Cari nama barang..." class="{{ $inputClass }}">
            @break

        @case('purchase_reason')
            <input wire:model.live.debounce.500ms="tableFilters.purchase_reason.value" type="search" placeholder="Cari alasan..." class="{{ $inputClass }}">
            @break

        @case('quantity')
            <input wire:model.live.debounce.400ms="tableFilters.quantity.value" type="number" min="1" placeholder="Cari qty..." class="{{ $inputClass }}">
            @break

        @case('purchase_type')
            <select wire:model.live="tableFilters.purchase_type.value" class="{{ $inputClass }}">
                <option value="">- Semua Jenis -</option>
                @foreach(PurchaseType::cases() as $type)
