...
<div class="fixed inset-x-0 bottom-0 z-30 flex border-t border-gray-100 bg-white/95 backdrop-blur-sm dark:border-gray-800 dark:bg-gray-900/95">
    ...
    <a href="{{ route('filament.casual.pages.purchase-request-page') }}"
       class="flex flex-1 flex-col items-center gap-1 py-3">
        @if($active === 'form')
            <svg class="h-6 w-6 text-violet-600" fill="currentColor" viewBox="0 0 24 24">
                <path fill-rule="evenodd" d="M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25ZM12.75 9a.75.75 0 0 0-1.5 0v2.25H9a.75.75 0 0 0 0 1.5h2.25V15a.75.75 0 0 0 1.5 0v-2.25H15a.75.75 0 0 0 0-1.5h-2.25V9Z" clip-rule="evenodd"/>
            </svg>
            <span class="text-xs font-semibold text-violet-600">Pengajuan</span>
        @else
    ...
    {{-- Riwayat --}}
    <a href="{{ route('filament.casual.pages.purchase-request-history-page') }}"
       class="flex flex-1 flex-col items-center gap-1 py-3">
        @if($active === 'history')
            <svg class="h-6 w-6 text-violet-600" fill="currentColor" viewBox="0 0 24 24">
                <path fill-rule="evenodd" d="M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25ZM12.75 6a.75.75 0 0 0-1.5 0v6c0 .414.336.75.75.75h4.5a.75.75 0 0 0 0-1.5h-3.75V6Z" clip-rule="evenodd"/>
            </svg>
            <span class="text-xs font-semibold text-violet-600">Riwayat</span>
        @else
            <svg class="h-6 w-6 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
