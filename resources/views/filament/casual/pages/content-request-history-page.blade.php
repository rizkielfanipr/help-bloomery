...
<div class="flex flex-col bg-blue-600 dark:bg-blue-900" style="min-height:100dvh">
    ...
    <div class="flex-shrink-0 px-5 pb-8 pt-14">
        <div class="mb-4 flex items-center gap-3">
            ...
            <span class="text-base font-semibold text-white">Riwayat Request Konten</span>
        </div>
    ...
    <div class="flex-1 overflow-y-auto rounded-t-3xl bg-gray-50 pb-28 pt-6 dark:bg-gray-950">
        ...
        @if($requests->isEmpty())
            <div class="flex flex-col items-center justify-center px-5 py-16 text-center">
                ...
                <p class="text-sm font-medium text-slate-600 dark:text-slate-400">Belum ada permintaan konten</p>
                <p class="mt-1 text-xs text-slate-400">Permintaan konten Anda akan muncul di sini</p>
        ...
        @else
            <div class="flex flex-col gap-3 px-5">
                @foreach($requests as $request)
                    @php
                        $statusValue = $request->status instanceof \App\Enums\ContentRequestStatus
                            ? $request->status->value : $request->status;
                        $statusLabel = $request->status instanceof \App\Enums\ContentRequestStatus
                        ...
                        <button wire:click="toggleItem({{ $request->id }})" type="button"
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <p class="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                                    {{ $request->jenis_konten === 'video' ? 'Video' : 'Foto' }} · {{ $request->platform_tujuan ?? '-' }}
                                </p>
                        ...
                        @if($isExpanded)
                            <div class="border-t border-gray-100 dark:border-gray-800">
                                ...
                                <div class="bg-white px-4 py-3 dark:bg-gray-900">
                                    <p class="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Tujuan Konten</p>
                                    <p class="mt-1 text-xs text-slate-700 dark:text-slate-300">{{ $request->tujuan_konten }}</p>
                                </div>

                                @if($request->link_contoh_konten)
                                    <div class="border-t border-gray-100 bg-white px-4 py-3 dark:border-gray-800 dark:bg-gray-900">
                                        <p class="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Link Contoh Konten</p>
                                        <a href="{{ $request->link_contoh_konten }}" target="_blank" class="mt-1 block truncate text-xs text-blue-600 dark:text-blue-400">{{ $request->link_contoh_konten }}</a>
                                    </div>
                                @endif
    ...
    <x-content-request.bottom-nav active="history" />
