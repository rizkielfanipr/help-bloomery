...
@once
    <span
        class="hidden"
        x-init="
            if (window.tripSelectionSyncInstalled) return
            window.tripSelectionSyncInstalled = true
...
<th class="fi-ta-header-cell align-top" @if($width) style="width: {{ $width }}" @endif>
    ...
    @switch($name)
        ...
        @case('driver.name')
            <select wire:model.live="tableFilters.driver.value" class="{{ $inputClass }}">
                <option value="">- Semua Driver -</option>
                @foreach(User::whereHas('roles', fn ($q) => $q->whereIn('name', ['DRIVER', 'driver', 'Driver']))->orderBy('name')->pluck('name', 'id') as $driverId => $driverName)
        ...
        @case('tripRoute.name')
            <input wire:model.live.debounce.500ms="tableFilters.route_name.value" type="search" placeholder="Cari rute/kendaraan..." class="{{ $inputClass }}">
            @break
        ...
        @case('trip_date')
            <div
                x-data="{
                    open: false,
                    from: $wire.entangle('tableFilters.trip_date.from').live,
                    until: $wire.entangle('tableFilters.trip_date.until').live,
                    draftFrom: null,
                    draftUntil: null,
                    hover: null,
                    cursor: new Date(),
        ...
        @case('fuel_summary')
            <select wire:model.live="tableFilters.has_fuel_fillup.value" class="{{ $inputClass }}">
                <option value="">- Semua BBM -</option>
                <option value="1">Ada Pengisian BBM</option>
                <option value="0">Tanpa BBM</option>
            </select>
            @break
    @endswitch
</th>
